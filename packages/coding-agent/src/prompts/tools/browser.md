...
- Other helpers: `url`, `title`, `ariaSnapshot` (read-only), `screenshot`; `click`, `type`, `fill`, `press("Enter" | "Control+a")`, `scroll("down", { by: "page" })`, `select`, `uploadFile`; `waitForSelector`, `waitForUrl`; `evaluate` (a string is a page-global expression — no top-level `return`). Input never selects the tab or focuses Chrome.
