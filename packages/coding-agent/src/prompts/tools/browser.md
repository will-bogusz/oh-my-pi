...
- Keep/release: ownership ends with your turn, pages do not. Every tab stays open and is handed back (task group and "OMP is debugging this browser" bar go away), so the user can take over a half-finished page and ask you to continue on it. Tabs you created close when the session ends unless you `await tab.keep()` — call it on results the user asked to see. Nothing carries over — next turn, claim the exact id again. `tab.release()` hands back early; `tab.close()` and `browser.closeTab(id)` physically close (even claimed or kept) and refuse another actor's tab.
