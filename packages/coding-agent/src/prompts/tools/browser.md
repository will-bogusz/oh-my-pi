...
- Keep/release: ownership ends with your turn. Created tabs close; claimed tabs and tabs you `await tab.keep()` stay open; the task tab group and "OMP is debugging this browser" bar go away. `keep()` is for results the user asked to see. Nothing carries over — next turn, claim the exact id again. `tab.release()` hands back early; `tab.close()` and `browser.closeTab(id)` physically close (even claimed or kept) and refuse another actor's tab.
