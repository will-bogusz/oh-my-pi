...
- `committed: false` after `setValue`: the app's own end-of-edit gesture did not run — re-read the field before building on it, and `type` the value if it stays uncommitted.
