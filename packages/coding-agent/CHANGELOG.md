...
- Managed Chrome tab ownership now ends when the agent hands control back: tabs OMP opened are closed, tabs it claimed from you and tabs marked `tab.keep()` stay open, the task tab group dissolves and the remaining `chrome.debugger` attachments detach so Chrome's debugging bar disappears. `tab.retain()` is renamed `tab.keep()` and no longer holds a lease across turns; the next turn re-claims the exact tab id. The `browser.releaseChromeOnSettle` setting is gone (always on).
