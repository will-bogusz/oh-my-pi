...
- The `computer` and `browser` tool prompts no longer enumerate their APIs: each states its entry points, the mental model (window/observation/ref/frame, delivery, what a result proves) and the hard boundaries, and leaves the method surface to the acquisition result's verb line and `computer.help()` / `browser.help()` (1 058 → 579 and 681 → 541 rendered words); schema changes no longer touch the prompts.
...
- Managed Chrome tab ownership now ends when the agent hands control back: every page stays open (created or claimed), the task tab group dissolves and the remaining `chrome.debugger` attachments detach so Chrome's debugging bar disappears. Nothing is closed automatically — the model closes the tabs it opened with `tab.close()` when it is done with them — so you can take over a half-finished page and ask the agent to continue on it. `tab.retain()`/`tab.keep()` are gone; the next turn re-claims the exact tab id. The `browser.releaseChromeOnSettle` setting is gone (always on).
- A freshly auto-started browser relay now holds its first request (up to 3 s) until the paired extension reconnects, so the first `browser.instances()`/`discover()` of a session no longer reports zero browsers.
- The `computer.releaseOnSettle` setting is gone: the driver child is always ended when the agent hands control back (the screen-sharing indicator clears); `computer.release()` remains for on-demand release.
- When Chrome revokes OMP's debugger on an open tab (another extension, typically a password manager, embedded its `chrome-extension://` UI in the page — common on sign-in forms), page calls and re-claims now say so and name the user step, instead of reporting a closed target.
- Child tabs a managed page opens are leased automatically by the browser service from Chrome's own popup path and listed by `tab.popups()`; the page-script `window.open`/`form.submit` interception is gone, so `window.open` returns a real `WindowProxy` and named windows and new-window form posts behave normally.
- `browser.getTab({title})`/`{url}` match case-insensitive substrings instead of whole strings, and `browser.tab(name)` now returns the managed Chrome handle registered under that name instead of an unusable alias.
- `tab.scroll` accepts a direction (`tab.scroll("down", { by: "page" })`) as well as pixel deltas; the direction form previously reached CDP as an invalid payload.
- `browser.refs` defaults to `compact` (`e1`…`eN` with self-healing refs); `uuid` remains selectable.
- `browser.relay=false` now really disables existing-Chrome discovery, claiming and closing rather than only `browser.open`; pass `app.relay: true` on a call to override it.
- Claiming a Chrome tab whose renderer is blocked by a JavaScript dialog is an ordinary claim that exposes the dialog; the separate pending-dialog acquisition mode and its side connection are gone.
- Removed the download saved-path lookup (`tab.downloads({paths:true})`) and with it the extension's optional download permission; `tab.downloads()` reports page-scoped ids, states and byte counts.
- Releasing a managed Chrome tab is now one explicit action: keeping a page hands it back to the user (out of the tab group, debugger detached, favicon restored, ownership dropped), and closing a tab takes it out of its group first, so no empty group or saved-group chip survives a task.
- Tabs OMP drives are marked with a cursor-glyph favicon while it holds them, and its tab group is one per owner, titled "Oh My Pi" unless the task names it.
- Tabs the browser opens from a driven tab are adopted from Chrome's own opener information instead of page-script interception, and are listed for their owner.
- The relay serves CDP only for an acquired tab; the legacy unscoped drive path with its target-claim and group RPCs, and the saved-download-path lookup with the extension's downloads permission, are gone.
- Native computer control now runs through a supervised `cua-driver mcp --direct` child spawned from the vendored executable (`vendor/cua-driver/<platform>/`), with per-request cancellation (`notifications/cancelled` / `cancel_operation`) instead of driver shutdown. The in-process Cua runtime, the Bun worker subprocess, the pi-natives computer backend, the machine-wide input file lock and the npm first-run install are removed; `computer.capabilities()` returns real values on its first call.
...
- The Code Mode declarations for `browser` and `computer` are cut to the verbs and types needed to write correct calls (19.4 KB → 7.3 KB and 10.9 KB → 7.0 KB), so an eval description with both surfaces on shrinks from 43.7 KB to 27.7 KB.
- Browser/computer observation stills follow a preview policy: an eval result carries one still per tab/window (the last displayed) to the model, and when the session settles the stills a later result superseded leave the model transcript. The rendered transcript keeps every image (`details.images`).
