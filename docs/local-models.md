...
- Auto-thinking, Smart unexpected-stop detection, typed Eval judgments, and AI-assisted git staging use the `judge` role. Assign `typesafe/jev-latest` for TypeSafe, `openrouter/~typesafe/jev-latest` for Jev billed to an OpenRouter key, or a compatible local tiny model for on-device judgment; order alternatives under `retry.fallbackChains.judge`.
