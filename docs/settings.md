...
The disabled-by-default `computer` Eval prelude observes and controls real host windows through a supervised `cua-driver` child spawned from the executable vendored for the host; see [The driver child](tools/computer.md#the-driver-child) and [Platforms](tools/computer.md#platforms). There is no backend setting or failure-driven switch. Window actions default to background with no automatic foreground retry; transient focus changes remain possible. Desktop-global input requires explicit foreground delivery. Use the separate `browser` Eval prelude for Chromium/CDP tabs and page automation.
...
| `computer.display`   | string  | `all`   | Desktop capture only. `all` and `primary` both select the primary display; any other selector is refused, and secondary displays and a multi-monitor composite are unsupported. |
...
| `computer.achieve`   | boolean | `false` | Experimental. Adds `win.achieve(goal)` to the prelude: a bounded sub-loop in which a typed judge picks the next action from a code-built candidate table, the driver executes it, and a postcondition judgment decides. Off, the method is absent and the host refuses it. Read at session start. |
...
| `providers.judgmentProvider`        | enum    | `auto`    | Preferred backend for typed judgments (auto-thinking difficulty, Smart unexpected-stop detection, git AI staging, eval `judge()`). `auto` uses TypeSafe when `TYPESAFE_API_KEY` or a `/login typesafe` credential exists, else Jev through OpenRouter's Decisions API when an OpenRouter credential exists; failed System One requests fall back through `tiny`, `smol`, `default`, then the active session model. `llm` skips both; a feature-configured local model remains the direct backend when System One is not selected.                                                                                                 |
