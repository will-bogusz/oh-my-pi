...
The disabled-by-default `computer` Eval prelude observes and controls real host windows in a dedicated child process. Apple Silicon macOS uses a pinned, patched Cua SDK for both capture and actions; other hosts use OMP's native desktop adapter. There is no backend setting or failure-driven switch. Window actions default to background with no automatic foreground retry; transient focus changes remain possible. Desktop-global input requires explicit foreground delivery. Use the separate `browser` Eval prelude for Chromium/CDP tabs and page automation.
...
| `computer.display`   | string  | `all`   | Desktop capture only. On Apple Silicon macOS, `all` and `primary` both select the primary display; secondary displays and a multi-monitor composite are unsupported. Other hosts use the native backend's `all` or display ID selection. |
| `computer.maxWidth`  | number  | `3840`  | Maximum desktop or window screenshot width in pixels. Image transports that cannot preserve original detail, including GitHub Copilot Responses and xAI OAuth, cap the effective width at `1280`; Claude-family models use the same cap as a compatibility fallback. |
| `computer.maxHeight` | number  | `2400`  | Maximum desktop or window screenshot height in pixels. Those coordinate-safe transports cap the effective height at `896`; other models retain the configured limit. |

Computer settings and the active model's image limits are read for every call; edits to settings files require a new session, while runtime setting changes apply to the next call. Select an explicit window, call `observe()` for structured AX and an image, act on current tokens, then observe or verify the effect. Pixel input requires the latest image of the same target; AX bounds are not image coordinates. Configure `tools.approvalMode` or `tools.approval.computer` and inspect host-owned permissions. No standalone Cua application or Codex/ZCode grants are required.

Cua window screenshots keep a rendering lease active and macOS shows its sharing indicator. Use `await computer.release()` when finished: it drains work and releases the lease, and later calls can start a fresh worker. `/computer off` also releases resources before reporting success and keeps computer use disabled until `/computer on`. Applications and their windows stay open. `computer.close()` retains its permanent behavior and requires a new OMP session afterward. See [Scriptable computer use](computer-use.md) for lifecycle and capability limits, and [runtime setup](tools/computer.md#runtime-setup) for the verified native payload and build instructions.
