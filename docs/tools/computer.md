...
| `achieve(goal, { maxSteps?, confidence? })` | Experimental, present only with `computer.achieve: true` (Python: `max_steps`, `confidence`). A bounded sub-loop: per step the host observes, builds a candidate table (enabled on-screen rows × their allowed actions; values only as the goal quotes them; destructive rows - delete, remove, discard, trash, quit - never), a typed judge picks a row or `reobserve`/`abstain`, the pick runs as `win.ref(r).<action>()` would, and a postcondition judgment over the fresh tree decides. Returns `{ done, steps, reason, abstained, detail? }` with the driver's replies verbatim; `reason` is `done`, `abstain`, `max_steps`, `interrupted` or `refused` |
